...
function ProductDetail({ currency, rates, language }) {
  ...
  const handleToggleLike = async () => {
    if (!token) return alert("Please login to like this product!");
  ...
  const handleAddToCart = async (sareeId) => {
    ...
    if (!token) return alert("Please login to add product to cart!");
    setIsAdding(true);
    try {
      ...
      if (response.ok) alert(`Added ${quantity} item(s) to cart!`);
